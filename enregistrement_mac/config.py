...
Meme fonctionnement que dans ExtractionMAC, volontairement : les deux outils se
posent cote a cote sur le poste de test et se comportent pareil.

Le fichier est ecrit **a cote du programme** : dans le dossier de
``EnregistrementMAC.exe``, ou a la racine du projet quand on lance les sources.
...
``%APPDATA%\\EnregistrementMac``. Sans ce repli, l'enregistrement des profils
...
NOM_FICHIER = "EnregistrementMAC-config.json"
...
CLES_BOOLEENNES = ("vider_cache_arp",)
...
def dossier_repli():
    ...
    if os.name == "nt":
        ...
        return os.path.join(base, "EnregistrementMac")
    base = os.environ.get("XDG_CONFIG_HOME") or os.path.join(os.path.expanduser("~"), ".config")
    return os.path.join(base, "enregistrement_mac")
